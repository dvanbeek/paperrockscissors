...
public class Game {
    ...
    public Game(CLI cli, Rules rules, Player player1, Player player2) {
        this.cli = cli;
        this.rules = rules;
    ...
    public void nextRound() {
        ...
        cli.summarizeRound(player1, player2, move1, move2, winnerString);
        if (cli.promptPlayAnotherRound()) {
            nextRound();
        } else {
            endGame();
        }
    ...
    public void endGame() {
        cli.summarizeGame(player1, player2, scoreboard[1], scoreboard[2], scoreboard[0]);
    }
}
